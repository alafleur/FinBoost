...
/**
 * DisbursementHistory
 * - Standalone admin page for browsing and exporting payout history.
 * - Lets admins pick ANY cycle, list all batches, view summaries, and export CSV.
 * - Expects these backend routes:
 *    GET  /api/admin/cycles                         -> [{id, name/title, ...}]
 *    GET  /api/admin/payout-batches?cycleId=ID     -> list batches for cycle
 *    GET  /api/admin/payout-batches/:id/summary    -> item summary + totals
 *    GET  /api/admin/payout-batches/:id/items.csv  -> CSV export of items
 */
...
export default function DisbursementHistory() {
  ...
  // Load batches when cycle changes
  useEffect(() => {
    if (!cycleId) return;
    (async () => {
      setLoading(true);
