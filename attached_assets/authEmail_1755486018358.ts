...
// Add this table per README step 1:
...
router.post('/verify/request', async (req, res) => {
  try {
    ...
    if (!user) return res.json({ success: true, message: 'If your email exists, we sent a email' });
    ...
    await db.insert(emailVerificationTokens).values({ userId: (user as any).id, token, expiresAt });
...
router.post('/password/request', async (req, res) => {
  try {
    ...
    await db.insert(passwordResetTokens).values({ userId: (user as any).id, token, expiresAt });
